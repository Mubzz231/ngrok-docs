...
quickly add authentication to your app with a few lines of code. First, stop
your app with Ctrl+C.

If your Google account is alan@gmail.com, you can restrict access only for
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let listener = ngrok::Session::builder()
        ...
        .http_endpoint()
        // highlight-start
        .oauth(OauthOptions::new("google").allow_email("alan@gmail.com"))
        // highlight-end
        .listen()
        .await?;
    println!("App URL: {:?}", listener.url());
...
- **Line 17**: Require visitors to authenticate with Google before they access your application. You can [choose other providers](/http/oauth/#supported-providers). Only allow `alan@gmail.com` access to the application

:::

Run your app again and visit the new URL it prints in the terminal.
...
## Step 5: Always use the same domain
...
then update your `ngrok.Listen` function call to use it:
...
- Read the [ngrok-rust package documentation on
  docs.rg](https://docs.rs/ngrok/latest/ngrok/)
