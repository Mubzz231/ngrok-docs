...
the internet and secure it so that only you can access it.
...
quickly add authentication to your app without any changes. First, stop ngrok
with `Ctrl+C`. If your Google account is `alan@gmail.com`, you can restrict
access only for yourself by running ngrok again with:

```
ngrok http http://localhost:8080 --oauth=google --oauth-allow-email=alan@gmail.com
```
...
ngrok, your app's URL changed, so make sure to use the new one.
...
then use the `--domain` flag to ask the ngrok agent to use it.
