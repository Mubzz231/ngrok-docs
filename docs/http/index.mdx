...
domains](/docs/network-edge/#branded-domains) for additional details.
...
wildcard domains](/network-edge/#wildcard-domains).
...
[By default, ngrok create an HTTPS endpoint but not an HTTP
...
[Read the Domains documentation](/network-edge/#domains) for additional details
about how ngrok chooses domains, what managed based domains ngrok operates, how
to set up branded domains, how wildcard domains work and more.
...
record](/docs/network-edge/#branded-domains). Once you've set one up, you can
...
| [Compression](compression)                   | Accelerate your application by automatically compressing HTTP response bodies with gzip or deflate.                       |
...
| [OAuth](oauth)                               | Enforces an OAuth authentication flow in front web applications, optionally authorizing users by domain or email address. |
| [OpenID Connect](openid-connect)             | Restrict access to web applications by enforcing an OpenID Connect authentication flow to an IdP.                         |
| [Request Headers](request-headers)           | Add and remove headers from HTTP requests before they are sent to your upstream service.                                  |
| [Response Headers](response-headers)         | Add and remove headers from HTTP responses before they are returned to the client.                                        |
| [SAML](saml)                                 | Restricts access to web applications by enforcing a SAML authentication flow with an IdP.                                 |
| [TLS](tls-termination)                       | Configure the minimum permitted version of TLS.                                                                           |
| [Webhook Verification](webhook-verification) | Assert requests to your endpoint are signed by a supported webhook provider like Slack or GitHub.                         |
